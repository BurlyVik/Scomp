...
namespace scomp
{
    public partial class Form1 : Form
    {
        ...
        private Color GetNextColor()
        {
            ...
            int highValue = 255; // Adjust as needed to ensure contrast.
        ...
        private void checksumLogToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private async void csvTxtToolStripMenuItem_Click(object sender, EventArgs e)
        {
            StringBuilder entitiesCsv = new StringBuilder();
            StringBuilder entitiesTxt = new StringBuilder();
            ...
            foreach (ListViewItem selectedItem in listView1.SelectedItems)
            {
                if (selectedItem.SubItems.Count > 1)
                {
                    entitiesCsv.AppendLine($"\"{selectedItem.Text}\",\"{selectedItem.SubItems[1].Text}\",\"{selectedItem.SubItems[2].Text}\",\"{selectedItem.SubItems[3].Text}\"");
                    entitiesTxt.AppendLine($"Filename: {selectedItem.Text}\nModified: {selectedItem.SubItems[1].Text}\nSize (bytes): {selectedItem.SubItems[2].Text}\nChecksum: {selectedItem.SubItems[3].Text}\n");
                }
            }
            ...
            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                if (saveFileDialog1.FilterIndex == 1)
                {
                    ...
                    {
                        stream.Write(entitiesTxt.ToString());
                    }

                }
                if (saveFileDialog1.FilterIndex == 2)
                {
                    File.WriteAllText(saveFileDialog1.FileName, "File,Modified,Size(bytes),Hash\n");
                    using (var stream = new StreamWriter(saveFileDialog1.FileName, true))
                    {
                        stream.Write(entitiesCsv.ToString());
                    }
                    // File.WriteAllText(saveFileDialog1.FileName, "File,Modified,Size(bytes),Hash\n" + entitiesCsv.ToString());
                }
            }

            MessageBox.Show($"File: {Path.GetFileName(saveFileDialog1.FileName)} will be saved to:\n{Path.GetDirectoryName(saveFileDialog1.FileName)}","Save File",MessageBoxButtons.OK,MessageBoxIcon.Information);
